...
/** Problem 11 http://projecteuler.net/problem=11
  * 
...
""" filter(_.toString match {
  ...
  println(ls.map(_.product).max)
  
  /** Define functions for getting 4-tuples ("quadruples") lined up
    * horizontally, vertically, or diagonally for 20x20 matrix
    * represented as a Vector (i.e., elements accessed via one index
    * instead of two).
    */
  
  def horizontalLines(matrix: Vector[Int])   = lines(matrix, 1)  // Horizontal gap in index values = 1
  ...
  def lines(matrix: Vector[Int], indexGap: Int) = {
    for {
      ...
      if (i + indexGap * 3 < matrix.length)
    } yield Vector(matrix(i), matrix(i + indexGap), matrix(i + indexGap * 2), matrix(i + indexGap * 3))
  } toVector
  
  /** Return a Vector that logically is used like a matrix */
